...
-- thống kê các đầu sách mượn nhiều --
select books.name AS book_name, count(borrows.id) AS borrows_count
from books
         join borrows ON books.id = borrows.book_id
group by books.name
...
WHERE borrows.book_id IS NULL;

-- thống kê các đầu sách chưa mượn --
select books.name AS book_name
from books
         left join borrows on books.id = borrows.book_id
WHERE borrows.book_id IS null;
